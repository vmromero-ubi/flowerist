...
from client import FlowerISTClient
...
def load_datasets(num_clients:int):
    ...
    Args:
        num_clients (int): The number of clients/partitions to create.

    Returns:
        trainloaders (list): A list of DataLoaders for training data in each partition.
        valloaders (list): A list of DataLoaders for validation data in each partition.
    ...
    testloader = DataLoader(testset, batch_size=32, shuffle=False) # pylint: disable=W0621
...
def client_fn(cid) -> FlowerISTClient:
    print(f"Client {cid}: Loading data")
    net = IndependentSubNetworks(
        layer_dims = [LAYER_DIMS[0] if i==0 else LAYER_DIMS[i]//PARTITON_NUM for i, dim in enumerate(LAYER_DIMS)],
        partition_num = 1,
        input_size=784,
    ...
    full_net = IndependentSubNetworks(
        layer_dims = LAYER_DIMS,
        partition_num = PARTITON_NUM,
        input_size=784,
        label_num=NUM_LABELS,
        track_bn_stats=False
    )
    ...
    return FlowerISTClient(cid, net, full_net, client_trainloader, client_valloader, DEVICE)
...
strategy = FedIST(
    ...
    model_partitions=PARTITON_NUM,
    conflict_resolution="aggregate",
    ist_type="legacy",
    layer_dims=LAYER_DIMS,
    label_num=NUM_LABELS,
)
